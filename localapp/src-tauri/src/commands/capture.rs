...
fn run_continuous_capture_loop(
    ...
    // 【LA002008 デバッグ】リトライ回数と待機時間のログ出力用
    eprintln!("[LA002008 DEBUG] 連続キャプチャループ開始: profile={:?}", profile);
    ...
    if start_from_beginning {
        ...
        if !profile.window_title_keyword.is_empty() {
            if let Err(e) = bring_window_to_front(&profile) {
                eprintln!("[LA002009] 先頭復帰: 最前面化失敗 {}", e);
            } else {
        ...
        for i in 1..=MAX_REVERSE_PAGES {
            ...
            if let Err(e) = turn_page_reverse(&mut enigo, &profile.page_turn_key) {
                eprintln!("[LA002009] 先頭復帰: ページ送りエラー {}", e);
                break;
            }
            // 画面遷移が安定するまで待機
            ...
            let current_image = match capture_window_image(&profile) {
                Ok(img) => img,
                Err(e) => {
                    eprintln!("[LA002009] 先頭復帰: キャプチャ失敗 {}", e);
                    break;
                }
            };
    ...
    loop {
        ...
        // --- 最前面化（LA002008: xcap では非最前面ウィンドウがキャプチャ不可のため常に実行）---
        if !profile.window_title_keyword.is_empty() {
            if let Err(e) = bring_window_to_front(&profile) {
                emit_progress(
                    &app_handle,
        ...
        if let Some(ref prev) = prev_image {
            ...
            if mse < MSE_THRESHOLD {
                ...
                eprintln!(
                    "[LA002010 DEBUG] page={} は同一ページと判定（{}回目）のため、再ページ送りを試みます",
...
fn capture_window_image(profile: &CaptureProfile) -> Result<image::RgbaImage, String> {
    ...
    for attempt in 1..=MAX_RETRIES {
        eprintln!("[LA002008 DEBUG] capture_window_image 試行 {}/{}", attempt, MAX_RETRIES);

        let windows = Window::all()
            .map_err(|e| format!("ウィンドウ一覧取得エラー: {}", e))?;

        if windows.is_empty() {
            eprintln!("[LA002008 DEBUG] 試行 {}/{}: ウィンドウ一覧が空", attempt, MAX_RETRIES);
        ...
        match target {
            Some(window) => {
                ...
                match window.capture_image() {
                    ...
                    Err(e) => {
                        eprintln!(
                            "[LA002008 DEBUG] 試行 {}/{}: キャプチャ失敗 '{}' ({}x{}) : {}",
                            attempt, MAX_RETRIES, title, window.width(), window.height(), e
                        );
                        if attempt < MAX_RETRIES {
                            eprintln!("[LA002008 DEBUG] 500ms 待機後にリトライ");
                            thread::sleep(Duration::from_millis(500));
                            continue;
                        }
                        return Err(format!(
            ...
            None => {
                eprintln!(
                    "[LA002008 DEBUG] 試行 {}/{}: ウィンドウ '{}' が見つからない",
