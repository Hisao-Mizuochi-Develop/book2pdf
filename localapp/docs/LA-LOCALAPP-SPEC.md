...
- backend API（FastAPI）経由で OCR 処理を実行し、検索可能 PDF を取得する（LA003006）
...
| HTTP クライアント | Tauri `tauri-plugin-http` | backend API との通信。`reqwest` を re-export し、`multipart`/`json` feature でファイルアップロードを実現（LA003006） |
...
3. 各機能のフロー
   ...
   - **PDF作成（backend API 経由）**: 画像フォルダ/ZIP 選択 → 保存ダイアログで PDF パス指定 → `run_backend_ocr` で ZIP 作成 → backend ジョブ作成 → アップロード → OCR → PDF ダウンロード（LA003006）
...
│       │   ├── pdf_searchable.rs # OCR 付き検索可能 PDF 生成（LA008009）
│       │   ├── trim.rs           # 画像トリミング
│       │   ├── zip.rs            # ZIP エクスポート
