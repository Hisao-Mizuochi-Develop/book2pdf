...
| 002005 | ウィンドウ指定キャプチャ＋コンテンツ領域自動トリミング | 2026-08-16 | 2026-08-17 | 実装（002008に統合） |
| 002007 | ウィンドウ指定キャプチャ実装のコンパイルエラー修正 | 2026-08-16 | 2026-08-16 | 不具合修正 |
| 002008 | ウィンドウ指定キャプチャ実装（xcap crate 版） | 2026-08-16 | 2026-08-17 | 実装 |
| 002008-1 | 連続キャプチャバグ修正（MSE計算 & 最前面化） | 2026-08-16 | 2026-08-16 | 不具合修正 |
| 002008-2 | プロファイルUI改善（デフォルト選択・表示・バリデーション） | 2026-08-16 | 2026-08-17 | 改善 |
| 002008-3 | 連続キャプチャ途中完了バグ修正（MSE同一ページ判定の猶予） | 2026-08-20 | 2026-08-20 | 不具合修正 |
| 002009 | 連続キャプチャの出力フォルダ指定とトリミング画面への引継ぎ | 2026-08-20 | 2026-08-21 | 実装 |
...
| 005004 | ZIP 作成進捗インジケーター追加 | 2026-08-20 | 2026-08-20 | 改善 |
...
| 008010 | Tauri invoke 引数修正・エンドツーエンド動作確認 | 2026-09-03 | 2026-09-04 | 実装 / 統合テスト |
| 008011 | OCR精度向上（Tesseract pre-processing / モデル選定） | 2026-09-04 |  | 改善調査 |
| 008012 | 透明テキストレイヤー・座標補正 | 2026-09-04 |  | 不具合修正 |
...
### 008006 backend OCR 連携機能実装

【計画】
- 008001〜008005 の個別実装を統合し、backend OCR 連携機能全体としての一貫性を確保する
...
> **注**: 本タスクは 2026-09-03 に再構成のため分割実施されました。新タスクは 008007（技術調査・選定）〜 008010（UI統合・収束）です。
...
- **再構成：上記内容は 008007（新）〜 008010 に分割して実施**

【実施結果】
- 2026-09-03: 再構成のため分割実施。詳細は 008007（新）〜 008010 を参照

### 008011 OCR精度向上（Tesseract pre-processing / モデル選定）
...
### 008008 localapp 単体PDF生成処理実装（旧）

> **注**: 本タスクは 2026-09-03 に再構成のため分割実施されました。新タスクは 008007（新）〜 008010（UI統合・収束）です。

【計画】
- 008007 で検討した技術選定に基づき、localapp 単体で画像フォルダ/ZIP から PDF を生成する Rust コマンドを実装する
- 生成パターン：
  - 画像結合 PDF（OCR なし）：画像をページとして結合した単純な PDF
  - 検索可能 PDF（OCR 付き）：008007 で選定した OCR エンジンを用いてテキストレイヤーを埋め込んだ PDF
