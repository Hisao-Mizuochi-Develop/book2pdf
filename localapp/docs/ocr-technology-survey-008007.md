# localapp 単体 OCR→PDF 技術調査レポート（タスク 008007）
...
- **localapp**: `printpdf` でテキスト描画。フォント埋め込みと透明色指定の方法が異なる。008009 で詳細調査が必要。
...
| 課題 | 次のタスク | 内容 |
|---|---|---|
| `image` crate デュアルバージョン | 008008 | compile-size 影響を測定し、ダウングレードか別名維持を決定 |
| `leptess` 統合 POC | 008009 | テスト PNG から OCR → `(text, bbox)[]` → PDF テキストレイヤー配置の一気通貫 |
| `printpdf` テキスト描画 | 008009 | 日本語フォント埋め込み、透明テキスト、座標変換の詳細調査 |
| ndlocr_cli との精度比較 | 008010 以降 | 同一画像セットで Tesseract vs ndlocr_cli の認識精度を比較評価 |
...
- **リスク**: `image` crate のデュアルバージョン。008008 で対応方針を確定する。
