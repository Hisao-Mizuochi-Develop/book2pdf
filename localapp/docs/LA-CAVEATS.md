...
### 関連タスク
- LA002005: ウィンドウ指定キャプチャ＋コンテンツ領域自動トリミング（実装途中で本問題を発見。2026-09-01 に LA002007 へ統合されたため完了）
- LA002006: ウィンドウ指定キャプチャ実装のコンパイルエラー修正（本問題の対応）
- LA002007: ウィンドウ指定キャプチャ実装（xcap crate 版）（LA002005 のウィンドウ指定キャプチャ機能を実質的に実現）
...
## 連続キャプチャ — 出力フォルダ指定とタブ間引継ぎ（LA002012）
...
## serde — Tauri イベントペイロードのフィールド名変換（TMP_LA002012_1）
...
### 関連タスク
- TMP_LA002012_1: 連続キャプチャの出力フォルダ指定とトリミング画面への引継ぎ（バグ修正）
...
| 対象 | 状態 | 備考 |
|---|---|---|
| Rust（Tauri コマンド） | ✅ 導入済み | `cargo test` で実行可能。`lopdf` を `[dev-dependencies]` に追加済み |
| localapp フロントエンド（React/Zustand） | ❌ 未導入 | vitest/jest 等のテストフレームワークがインストールされていない |
...
- 本来の UUID v4（RFC 4122）ではなく、ナノ秒タイムスタンプ + アトミックカウンターの簡易実装
- 関数コメントに「UUID v4 ではない」ことを明記済み
- 高速連続呼び出しでも一意性を保証するため、`AtomicU64` カウンターを追加した
- 暗号学的な一意性は不要（一時フォルダ名の衝突回避のみが目的）

### 今後の検討事項

- localapp フロントエンドに vitest を導入し、Zustand ストアの単体テストを追加する
- `extract_zip_to_temp` の単体テストは ZIP 生成が複雑なため、統合テストレベル（フルフロー）でカバーする方針
