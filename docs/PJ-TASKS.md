...
| [PJ001020](#pj001020) feature ブランチの削除判断・実行をユーザーに委ねるよう `.clinerules`・AgentSkills を改修 | 2026-09-10 | | 運用整備 |
...
>
> 【実施結果】
> - 2026-09-10: `.clinerules` に「タスク完了基準」「マージ後の追加チェック」「ユーザー承認の形式」「コミット・マージとタスク完了の厳密な分離」「タスク完了日付記入前の自己確認リスト」を追加
> - 2026-09-10: `.cline/skills/task-manager/SKILL.md` に「タスク完了前のユーザー承認取得（手順9）」を追加
> - 2026-09-10: `.cline/skills/workflow-runner/SKILL.md` Phase 4 を改修し、マージ後承認取得を必須化
> - 2026-09-10: `python scripts/lint-task-md.py` 37ファイル ALL PASS
> - 2026-09-10: main ブランチへ `--no-ff` マージ完了
> - 2026-09-10: マージ後承認をユーザーから取得、タスク完了日付を記入
>
<a id="pj001020"></a>
### PJ001020 feature ブランチの削除判断・実行をユーザーに委ねるよう `.clinerules`・AgentSkills を改修

<div align="right"><a href="#pj001">タスク一覧へ↩︎</a></div>

> 【計画】
> - `.clinerules` の「タスク完了基準」から feature ブランチの削除に関する条件を完全に外す
> - `.clinerules` に「feature ブランチの削除」新規セクションを追加する
>   - ブランチが不要かどうかの判断はユーザーが行う
>   - マージ後もブランチはユーザーが「不要」と判断するまで保持する
>   - AI が「マージしたから削除しよう」と勝手に提案・実行してはならない
>   - ユーザーから「ブランチを削除してください」と明示的に指示された場合のみ削除可能
>   - 次の指示ではブランチ削除してはならない
>     - git のコミット
>     - git のマージ
>     - タスクの完了
>   - タスクの完了指示があった場合は、対象のブランチの削除コマンドをユーザーに提示する（実行はしない）
> - `.cline/skills/branch-manager/SKILL.md` に「ブランチ削除はユーザー判断・実行」と明記する
> - `.cline/skills/workflow-runner/SKILL.md` の Git 運用手順にブランチ削除の注意を追加する
> - `python scripts/lint-task-md.py` でパイプテーブル整合性を検証する
>
> 【実施結果】
