...
Windows専用だった book2pdf を Apple Silicon Mac に移植した際の技術的な変更点と、
...
移植中の実機テストで、電子書籍アプリが閉じている状態にもかかわらず、たまたま
作業ディレクトリ名に "電子書籍" を含むターミナルのウィンドウタイトル
（`book2pdf_mac — -zsh — ...`）を誤って選んでしまう事例を確認しました。
