...
## 2026-09-09 OT010003 `docs/SY-TASKS.md` の「識別子運用ルール」セクション削除と `.clinerules` 参照整理

### 目的

`docs/SY-TASKS.md` に記載されていた「識別子運用ルール」セクションが、`.clinerules` の **Task Identifier Rules** と重複していたため、`.clinerules` を正とし `docs/SY-TASKS.md` のセクションを削除する。

### 実施内容

- `docs/SY-TASKS.md` から「識別子運用ルール」セクションを削除
- `docs/PJ-TASKS.md` の `PJ001001` 【計画】・【実施結果】から「識別子運用ルール明記」の記述を削除し、タスク粒度の方針明記のみに整理
- `docs/OT-TASKS.md` に `OT010003` を起票

### 結果

- 識別子ルールの一元管理が `.clinerules` に集中し、重複が解消された
- `docs/SY-TASKS.md` はタスク粒度の方針とユースケース記録のみを保持する形になった

---

## 2026-09-09 OT010004 各 `<識別子>-TASKS.md` の「タスク粒度の方針」を自識別子のみの記述に統一

### 目的

各 `<識別子>-TASKS.md` の「タスク粒度の方針」セクションで、他のモジュール識別子に言及していた記述を排除し、自識別子のみを記載する形に統一する。

### 実施内容

- `docs/SY-TASKS.md`: `SY` のみに統一（`BE`/`FE`/`OW`/`LA`/`OT` の列挙を削除）
- `docs/OT-TASKS.md`: `OT` のみに統一（`SY`/`PJ`/`BE`/`FE`/`OW`/`LA` の列挙を削除）
- `frontend/docs/FE-TASKS.md`: `FE` のみに統一（`SY` の言及を削除、例を 1 つに）
- `localapp/docs/LA-TASKS.md`: `LA` のみに統一（`SY` の言及を削除）
- `backend/docs/BE-TASKS.md`: `BE` のみに統一（`SY` の言及を削除、例を 1 つに）
- `ocr-worker/docs/OW-TASKS.md`: `OW` のみに統一（`SY` の言及を削除）
- `docs/OT-TASKS.md` に `OT010004` を起票

### 結果

- 7 ファイルすべての「タスク粒度の方針」が自識別子のみの記述に統一された
- `.clinerules` を正とする運用が各タスク管理ファイルにも反映された
