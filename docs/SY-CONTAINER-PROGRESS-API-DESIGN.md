...
### 7.1 ocr-worker（Phase 1）

1. `ndlocr_cli_patches/progress_reporter.py` の変更:
...
1. `backend/tests/test_progress.py` の更新:
   - ocr-worker HTTP API モックを使ったマージテスト
   - オフライン時のフォールバックテスト

2. `frontend` の更新:
   - backend の SSE 出力形式に変更がないため原則変更なし
   - 必要に応じて統合テストを更新
