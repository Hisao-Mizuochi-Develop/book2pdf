#!/bin/bash
# OW003005 再検証スクリプト（前処理 ON 状態で score_thr パターン A/B/C を backend API 経由で実行）
set -euo pipefail

ZIP_FILE="benchmark-ocr-OW003002.zip"
...
for i in "${!PATTERNS[@]}"; do
    ...
    # 成果物取得
    mkdir -p "ocr-results-OW003005/${name}-preprocess-on/pdfs"
    docker compose cp "backend:/data/pdfs/${JOB_ID}.pdf" "ocr-results-OW003005/${name}-preprocess-on/pdfs/"
    ...
    mkdir -p "ocr-results-OW003005/${name}-preprocess-on/output"
    docker compose cp "backend:${OUTPUT_DIR}/" "ocr-results-OW003005/${name}-preprocess-on/output/"

    echo "$JOB_ID" > "ocr-results-OW003005/${name}-preprocess-on/job_id.txt"
    curl -s "http://localhost:8000/api/jobs/$JOB_ID" | jq . > "ocr-results-OW003005/${name}-preprocess-on/job_info.json"
