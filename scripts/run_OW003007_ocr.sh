#!/bin/zsh
# OW003007: 追加前処理効果検証の OCR フルフロー自動実行スクリプト
...
RESULTS_DIR="ocr-results-OW003007"
mkdir -p "$RESULTS_DIR"

# 比較パターン定義（パターン名=ZIPファイル名）
declare -A PATTERN_ZIPS=(
  [baseline_2x]="benchmark-ocr-OW003007-baseline-2x.zip"
  [4x_upscale]="benchmark-ocr-OW003007-4x-upscale.zip"
  [4x_upscale_sharpen]="benchmark-ocr-OW003007-4x-upscale-sharpen.zip"
  [local_binarization]="benchmark-ocr-OW003007-local-binarization.zip"
  [local_binarization_sharpen]="benchmark-ocr-OW003007-local-binarization-sharpen.zip"
  [contrast_strong]="benchmark-ocr-OW003007-contrast-strong.zip"
  [contrast_strong_4x]="benchmark-ocr-OW003007-contrast-strong-4x.zip"
)
...
for pattern in "${(@k)PATTERN_ZIPS}"; do
  ...
  echo "$ocr_response" | jq . > "$pattern_dir/ocr_response.json"
  ...
  # 6. OCR 出力ディレクトリをコピー（extracted ボリューム内の output ディレクトリ）
  echo "Copying OCR output..."
  output_dir=$(jq -r '.output_dir' "$pattern_dir/ocr_response.json")
