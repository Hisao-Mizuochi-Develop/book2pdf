...
| パイプ数は合っているが、同一テーブル内でデータが誤った列に配置されている | 各テーブル行の「空でないセルの列インデックス」を確認し、他の行と同じ列にデータが配置されるよう空セルを挿入・削除して調整する |
...
- 詳細行での列配置ずれ: `*-TASKS.md` のタスク詳細行などではテキストが 2 列目など特定の列に配置されるのが標準だが、誤って後続の列に入力されることがある。パイプ数だけでは検出できないため、レンダリングの視覚確認や「空でないセルの列インデックス」解析が必要