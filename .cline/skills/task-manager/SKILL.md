...
9. **タスク完了前のユーザー承認取得**
   - マージ完了後であっても、`.clinerules` の「マージ後の追加チェック」に従い、【タスク完了日付】を記入する前に以下を確認する
     - ユーザーにマージ完了報告と変更内容を提示する
     - 「タスク完了として【タスク完了日付】を記入してよろしいでしょうか？」と明確に尋ねる
     - ユーザーの回答は文脈に応じて解釈する。タスク完了を認める意図が明確に伝わる発言は承認とみなす
     - 操作指示のみの発言（「マージしました」「コミットしてください」など）は完了承認とみなさない
   - 承認を得た後で初めて【タスク完了日付】を記入し、コミットする
