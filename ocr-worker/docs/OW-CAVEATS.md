...
## 22. 対象ページに応じた適応的前処理（OW003008）

- OW003003・OW003007 の検証結果から、ページタイプによって最適な前処理が異なる可能性が示唆された
- しかし、ページ分類器の実装コスト、再 OCR による処理時間増加、レイアウト認識結果への依存などを考慮し、現時点では導入を保留とした
- 技術書の表紙は 1 ページ、目次も少数ページに限られるため、一律前処理を維持する方がシンプル
- 過去の検証結果ディレクトリ（`ocr-results-OW003003/`、`ocr-results-OW003007/` など）はクリーンアップにより失われる可能性があるため、重要な結果はワークログや永続的なレポートに残す
