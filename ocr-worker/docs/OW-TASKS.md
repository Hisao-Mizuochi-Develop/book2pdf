...
| ユースケースNo | タイトル |
|---|---|
| [OW001](#ow001) | ndlocr_cli を実行可能な Docker コンテナ（ocr-worker）を構築する |
| [OW002](#ow002) | 性能計測に必要な DEBUG ログ・計測処理を追加する |
| [OW003](#ow003) | OCR 読み取り精度向上 |
| [OW004](#ow004) | OCR 処理の安定性・スケーラビリティ向上（ページ数に依存しない一定のメモリ使用量を実現する） |
| [OW005](#ow005) | OCR 認識精度を次世代アプローチで向上させる |
| [OW006](#ow006) | OCR 処理中のページ単位進捗通知精度を向上させる |
...
|---|---|---|---|
| [OW001001](#ow001001) ndlocr_cli 実行環境の Docker コンテナ化 | 2026-08-11 | 2026-08-11 | 機能実装 |
| [OW001002](#ow001002) ocr-worker OCR API の実装 | 2026-08-11 | 2026-08-11 | 機能実装 |

<a id="ow001001"></a>
### OW001001 ndlocr_cli 実行環境の Docker コンテナ化
<div align="right"><a href="#ow001">タスク一覧へ↩︎</a></div>
...
<a id="ow001002"></a>
### OW001002 ocr-worker OCR API の実装
<div align="right"><a href="#ow001">タスク一覧へ↩︎</a></div>
...
| タスク | タスク起票日付 | タスク完了日付 | タスク種別 |
|---|---|---|---|
| [OW002001](#ow002001) ocr-worker DEBUG ログ・計測処理の追加 | 2026-08-12 | 2026-08-12 | 機能実装 |
| [OW002002](#ow002002) OCR 処理性能計測の実施 | 2026-08-12 | 2026-09-20 | 性能評価 |

<a id="ow002001"></a>
### OW002001 ocr-worker DEBUG ログ・計測処理の追加
<div align="right"><a href="#ow002">タスク一覧へ↩︎</a></div>
...
>
> 【実施結果】
> - 2026-09-20: OT012001 棚卸しにより、本タスクを完了扱いに更新。性能計測用の DEBUG ログ・計測処理は OW002001 で実装済み
>
...
| タスク | タスク起票日付 | タスク完了日付 | タスク種別 |
|---|---|---|---|
| [OW003001](#ow003001) ocr-worker OCR 実行時 500 エラーの原因調査・修正（OCR 精度向上前の環境不具合修正） | 2026-08-13 | 2026-08-13 | 不具合修正 |
| [OW003002](#ow003002) 現状 OCR 認識精度の再測定 | 2026-08-13 | 2026-08-13 | 調査 |
| [OW003003](#ow003003) 入力画像前処理の効果検証 | 2026-08-13 | 2026-08-13 | 改善調査 |
| [OW003004](#ow003004) config.yml パラメータ調整の効果検証 | 2026-08-13 | 2026-08-14 | 改善調査 |
| [OW003005](#ow003005) OCR精度向上の統合検討と実装修正（config.yml score_thr 無視問題の修正と再検証） | 2026-08-13 | 2026-08-14 | 不具合修正 |
| [OW003006](#ow003006) sharpen_light_upscale_2x 自動前処理の ocr-worker 組み込み | 2026-08-14 | 2026-08-14 | 機能実装 |
| [OW003007](#ow003007) 追加前処理（4x アップスケール、局所的二値化、コントラスト強調など）の効果検証 | 2026-08-14 | 2026-08-15 | 改善調査 |
| [OW003008](#ow003008) 対象ページに応じた適応的前処理の検討 | 2026-08-15 | 2026-09-01 | 要検討対応 |

<a id="ow003001"></a>
### OW003001 ocr-worker OCR 実行時 500 エラーの原因調査・修正（OCR 精度向上前の環境不具合修正）
<div align="right"><a href="#ow003">タスク一覧へ↩︎</a></div>
...
<a id="ow003004"></a>
### OW003004 config.yml パラメータ調整の効果検証
<div align="right"><a href="#ow003">タスク一覧へ↩︎</a></div>
...
<a id="ow003005"></a>
### OW003005 OCR精度向上の統合検討と実装修正（config.yml score_thr 無視問題の修正と再検証）
<div align="right"><a href="#ow003">タスク一覧へ↩︎</a></div>
...
<a id="ow003006"></a>
### OW003006 sharpen_light_upscale_2x 自動前処理の ocr-worker 組み込み
<div align="right"><a href="#ow003">タスク一覧へ↩︎</a></div>
...
<a id="ow003007"></a>
### OW003007 追加前処理（4x アップスケール、局所的二値化、コントラスト強調など）の効果検証
<div align="right"><a href="#ow003">タスク一覧へ↩︎</a></div>
...
| タスク | タスク起票日付 | タスク完了日付 | タスク種別 |
|---|---|---|---|
| [OW004001](#ow004001) OCR 処理のメモリ使用量安定化 | 2026-08-27 | 2026-08-27 | 不具合修正 |

<a id="ow004001"></a>
### OW004001 OCR 処理のメモリ使用量安定化
<div align="right"><a href="#ow004">タスク一覧へ↩︎</a></div>
...
| タスク | タスク起票日付 | タスク完了日付 | タスク種別 |
|---|---|---|---|
| [OW005001](#ow005001) OCR 認識精度向上の次世代アプローチ調査 | 2026-09-03 |  | 調査 / 要検討対応 |

<a id="ow005001"></a>
### OW005001 OCR 認識精度向上の次世代アプローチ調査
<div align="right"><a href="#ow005">タスク一覧へ↩︎</a></div>
...
| タスク | タスク起票日付 | タスク完了日付 | タスク種別 |
|---|---|---|---|
| [OW006001](#ow006001) ページ単位進捗通知精度の向上 | 2026-09-08 | 2026-09-20 | 機能実装 |
| [OW006002](#ow006002) ocr-worker progress endpoint の import パス修正 | 2026-09-13 | 2026/09/13 | バグ修正 |

<a id="ow006001"></a>
### OW006001 ページ単位進捗通知精度の向上
<div align="right"><a href="#ow006">タスク一覧へ↩︎</a></div>
...
>
> 【実施結果】
> - 2026-09-20: OT012001 棚卸しにより、本タスクを完了扱いに更新
> - 計画の各項目は以下のタスクで実質的に実装済み：
>   - SY002002: ocr-worker per-page 進捗の in-memory 化、`GET /progress/{job_id}`、backend 側の in-memory 進捗とのマージ
>   - SY002003: `current_page` を 1-based に統一、frontend のページ単位タイミング抽出（`extractActualPage`）
>   - FE002001 / SY002003: frontend ProgressPanel にページ進捗メッセージ（`OCR処理中です（1/4）`）を表示
>

<a id="ow006002"></a>
### OW006002 ocr-worker progress endpoint の import パス修正
<div align="right"><a href="#ow006">タスク一覧へ↩︎</a></div>
