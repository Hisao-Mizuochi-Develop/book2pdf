...
|  | 各パターンで OCR を実行し、結果を `ocr-results-003002/` に保存する |  |  |  |
...
|  | |---|---|---|  |  |  |
|  | | baseline | 前処理なし | 比較基準 |  |  |  |
|  | | sharpen_light | 軽度シャープニング | 輪郭を鮮明化、英数字・漢字部品類似誤認識を抑制 |  |  |  |
|  | | sharpen_light_upscale_2x | 2 倍アップスケーリング＋軽度シャープニング | 文字サイズを大きくしつつ輪郭を補強 |  |  |  |
|  | | contrast_gamma | コントラスト強調＋ガンマ補正 | 薄字・記号の認識率向上 |  |  |  |
|  | | contrast_gamma_sharpen_light | コントラスト強調＋ガンマ補正＋軽度シャープニング | コントラストと輪郭の両方を改善 |  |  |  |
|  | | denoise | ノイズ除去 | スキャンノイズ抑制（今回は実施しない） |  |  |  |
|  | #### 3. OCR 実行 |  |  |  |
...
|  | 【実施結果】 |  |  |  |
|  | 2026-08-13: Docker 環境の不具合を修正し、OCR 実行実行時の 500 エラーが解消された |  |  |  |
