...
| 003004 | config.yml パラメータ調整の効果検証 | 2026-08-13 | 2026-08-14 | 改善調査 |
|  | タスク詳細 |  |  |  |
...
|  | 2026-08-13: **※後述の 003005 で判明：ndl_layout submodule の `process_textblock.py` / `process.py` に `score_thr: float = 0.3` がハードコードされており、config.yml の値が無視されていたため、本タスクのパラメータ変更は実質的に検証になっていなかった** |  |  |  |
|  | 2026-08-13: 精度比較レポート [ocr-results-003004/config-comparison-report-003004.md](ocr-results-003004/config-comparison-report-003004.md) を作成した |  |  |  |
|  | 2026-08-13: `ocr-worker/docs/work_log.md` に本タスクの作業ログを追記した |  |  |  |
|  | 2026-08-13: タスク完了日付を記載 |  |  |  |
| 003005 | OCR精度向上の統合検討と実装修正（config.yml score_thr 無視問題の修正と再検証） | 2026-08-13 | 2026-08-14 | 不具合修正 |
|  | タスク詳細 |  |  |  |
|  | 【計画】 |  |  |  |
...
|  | 3. `benchmark-ocr-003002.zip` を backend API 経由でアップロード・OCR 実行し、ジョブが `completed` になるまでポーリングする |  |  |  |
|  | 4. 生成された PDF を `docker compose cp backend:/data/pdfs/<job_id>.pdf ./ocr-results-003005/<pattern>/pdfs/` で取得する |  |  |  |
|  | 5. ocr-worker 出力（XML / txt）もあわせて取得し、`./ocr-results-003005/<pattern>/` に保存する |  |  |  |
|  | 6. 3 パターン分の PDF を目視確認し、レイアウト・文字認識・欠落行の違いを比較する |  |  |  |
|  | 7. `ocr-results-003005/config-comparison-report-003005.md` を作成する |  |  |  |
|  | 8. tasks.md の【実施結果】欩末にレポートリンクを追加し、タスク完了日付を記載する |  |  |  |
|  | ※前提：003003 で sharpen_light_upscale_2x が最も効果的、003004 で config.yml 調整に効果なし（ハードコーディングが原因と判明） |  |  |  |
|  |  |  |  |  |
...
|  |    - 003004 の検証パターン（score_thr 0.2 / 0.1 / 0.2+additional_elements False）を再実施し、修正後の効果を評価 |  |  |  |
|  |  |  |  |  |
|  | 2. 自動前処理統合の実装： |  |  |  |
...
|  |    - `benchmark-ocr-003002.zip` で動作確認：前処理ON/OFF の結果を比較し、003003 と同等の精度向上を確認 |  |  |  |
|  |  |  |  |  |
...
|  | 2026-08-14: 精度比較レポート [ocr-results-003005/config-comparison-report-003005.md](ocr-results-003005/config-comparison-report-003005.md) を作成した |  |  |  |
...
| 003006 | sharpen_light_upscale_2x 自動前処理の ocr-worker 組み込み | 2026-08-14 | 2026-08-14 | 機能実装 |
|  | タスク詳細 |  |  |  |
|  | 【計画】 |  |  |  |
|  | 目的：003003 で最も効果的だった `sharpen_light_upscale_2x` 前処理を ocr-worker 内部で自動適用し、全ページに対して精度向上を図る |  |  |  |
|  | 前提：003003 で `sharpen_light_upscale_2x` が最も効果的であることが確認済みであること |  |  |  |
|  | 前提：`benchmark-ocr-003002.zip`（002.png, 003.png, 004.png）をテストデータとして使用する |  |  |  |
...
|  |    - `PREPROCESS_ENABLED=true`（デフォルト）で `benchmark-ocr-003002.zip` を OCR 実行する |  |  |  |
|  |    - `PREPROCESS_ENABLED=false` にして ocr-worker を再起動し、同一 ZIP を OCR 実行する |  |  |  |
|  |    - 前処理 ON/OFF の「〓」出現数・誤認識箇所数を比較し、003003 と同等の効果を確認する |  |  |  |
...
|  | 2026-08-14: 前処理 OFF のジョブ `e07faa8c-195b-4d36-bc7c-d44e0aa28582` は status: completed となり、003002 baseline と同等の「〓」出現数 5個を確認した |  |  |  |
|  | 2026-08-14: 精度比較レポート [ocr-results-003006/preprocess-integration-report-003006.md](ocr-results-003006/preprocess-integration-report-003006.md) を作成した |  |  |  |
|  | 2026-08-14: `ocr-worker/docs/work_log.md` / `ocr-worker/docs/caveats.md` / `ocr-worker/docs/ocr-worker-system-spec.md` を更新した |  |  |  |

| 003007 | 追加前処理（4x アップスケール、局所的二値化、コントラスト強調など）の効果検証 | 2026-08-14 | 2026-08-15 | 改善調査 |
|  | タスク詳細 |  |  |  |
|  | 【計画】 |  |  |  |
|  | 目的：003003 で `sharpen_light_upscale_2x`（2倍アップスケール＋軽度シャープニング）が最も効果的だったが、表紙ページなどで残存誤認識があったため、さらに強力な前処理パターンの効果を定量的に検証する |  |  |  |
...
|  | 2. 001.png 〜 010.png を ZIP にまとめた `benchmark-ocr-003007.zip` を作成する |  |  |  |
...
|  | 5. `docker compose cp` で各ジョブの PDF および ocr-worker 出力（XML / txt）を `ocr-results-003007/<pattern>/` に取得する |  |  |  |
|  | 6. 各パターンの「〓」出現数、明らかな誤認識箇所数、目視確認結果を集計する |  |  |  |
|  | 7. 精度比較レポート `ocr-results-003007/additional-preprocess-report-003007.md` を作成する |  |  |  |
...
|  | 2026-08-14: `benchmark-ocr-003007.zip`（001.png〜010.png）と、7 パターンの前処理済み ZIP を作成した |  |  |  |
...
|  | 2026-08-15: 7 パターンすべての OCR が完了し、PDF・OCR 出力（XML/txt）を `ocr-results-003007/<pattern>/` に取得した |  |  |  |
...
|  | 2026-08-15: 精度比較レポート [ocr-results-003007/additional-preprocess-report-003007.md](ocr-results-003007/additional-preprocess-report-003007.md) を作成した |  |  |  |
|  | 2026-08-15: `ocr-worker/docs/work_log.md` に 003007 の実施実績を追記した |  |  |  |
...
|  | 結論：今回試した追加前処理（4x アップスケール、局所的二値化、強コントラスト）は、003006 で採用済みの 2x アップスケール＋軽度シャープニングを超える明確な改善は確認できず、新たな前処理として追加導入することは推奨されない |  |  |  |
...
| タスクNO | タスクタイトル | タスク起票日付 | タスク完了日付 | タスク種別 |
|---|---|---|---|---|
| 003008 | 対象ページに応じた適応的前処理の検討 | 2026-08-15 | 2026-09-01 | 要検討対応 |
|  | タスク詳細 |  |  |  |
|  | 【背景】 |  |  |  |
|  | 003007 の検証結果より、表紙・目次・本文などページタイプによって最適な前処理が異なる可能性が見られた。一律の前処理では全ページタイプに最適ではない。 |  |  |  |
|  | 【検討内容】 |  |  |  |
...
|  | 2026-09-01: 検討レポート [ocr-results-003008/adaptive-preprocess-report-003008.md](ocr-results-003008/adaptive-preprocess-report-003008.md) を作成し、上記結論を維持 |  |  |  |
