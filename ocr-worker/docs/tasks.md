...
|  | cd /Users/hisao/Documents/work4/sakura/book2pdf |  |  |  |
|  | mkdir -p scripts |  |  |  |
|  | cat > scripts/preprocess_image.py << 'EOF' |  |  |  |
|  | # 実装予定 |  |  |  |
|  | EOF |  |  |  |
|  | ``` |  |  |  |
|  | #### 2. 前処理パターンの定義 |  |  |  |
...
|  | | sharpen_light + upscale_2x | 2 倍アップスケーリング＋軽度シャープニング | 文字サイズを大きくしつつ輪郭を補強 |  |  |  |
|  | | contrast_gamma | コントラスト強調＋ガンマ補正 | 薄字・記号の認識率向上 |  |  |  |
|  | | contrast_gamma + sharpen_light | コントラスト強調＋ガンマ補正＋軽度シャープニング | コントラストと輪郭の両方を改善 |  |  |  |
|  | | denoise | ノイズ除去 | スキャンノイズ抑制 |  |  |  |
|  | #### 3. OCR 実行 |  |  |  |
