...
2. **4x アップスケールは処理時間とファイルサイズが増大し、精度は改善しない**
   ...
   - 4x_upscale_sharpen は baseline より改善（178）だが、003005 の 2x アップスケール＋シャープニングと比較して大きな差はない。
...
4. **全体として、今回試した追加前処理は 003006 で採用済みの 2x アップスケール＋軽度シャープニングを超える明確な改善は見られなかった**
   - 4x_upscale_sharpen や contrast_strong_4x で若干の改善はあるが、処理コスト（時間・ファイルサイズ）に見合う効果ではない。

## 結論

- 局所的二値化、4x アップスケール、強コントラストなどの追加前処理は、003006 で採用済みの 2x アップスケール＋軽度シャープニングを超える効果は確認できなかった。
...
4. **レイアウト認識パラメータとの組み合わせ調整**
   - 003005 で調整した `score_thr` パラメータとの組み合わせで、さらなる改善の余地があるか確認する。
