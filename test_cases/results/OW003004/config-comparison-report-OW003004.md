# OW003004 config.yml パラメータ調整効果検証 レポート

## 概要

- タスク: OW003004「config.yml パラメータ調整の効果検証」
- 実施日: 2026-08-13
- 対象データ: `benchmark-ocr-OW003003-sharpen-upscale.zip`（002.png, 003.png, 004.png）
  ※OW003003 で最も効果的だった `sharpen_light_upscale_2x` 適用済み画像（1328×1884）
...
|---|---|---|
| baseline | OW003003 の `sharpen_light_upscale_2x` 結果を流用 | なし（score_thr: 0.3） |
...
- OW003003 の `sharpen_light_upscale_2x` は「〓」を 5→3 に減少させ、誤認識を大幅に改善した
- OW003004 の config.yml 調整は**すべてのパターンでベースラインと同一の結果**となり、**改善効果なし**
